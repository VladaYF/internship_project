...
def df_filter_brand(brand_data):
    ...
    # drop empty
    storage_error['empty_value'] = list(
        *np.where(brand_data['brand_id'] == '')) + list(*np.where(brand_data['brand'] == ''))
    ...
    brand_data.drop(
        brand_data.iloc[storage_error['empty_value']].index, inplace=True)

    storage_error['not_number'] = np.where(
        ~brand_data['brand_id'].str.isdigit())

    df_error = df_error.append(brand_data.iloc[storage_error['not_number']])
    df_error['error'] = df_error['error'].fillna('not_number')

    brand_data.drop(
        brand_data.iloc[storage_error['not_number']].index, inplace=True)
