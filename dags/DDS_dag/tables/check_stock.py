...
def df_filter_stock(data, product_primary_key):
    ...
    # drop empty
    storage_error['empty_value'] = list(*np.where(data['product_id'] == '')) + list(*np.where(data['pos'] == '')) + list(*np.where(data['available_on'] == '')) + list(*np.where(data['cost_per_item'] == ''))
    ...
    # not_in_primary_key
    not_in_primary_key = np.where(~data['product_id'].isin(product_primary_key))[0]
    ...
    # not_int
    storage_error['not_number'] = list(*np.where(~data['product_id'].str.isdigit())) + list(*np.where(~data['available_on'].str.isdigit())) + list(*np.where(~data['available_quantity'].str.isdigit()))
    ...
    storage_error['not_float_or_int'] = list(*np.where(data['product_id'].notna() == False))
